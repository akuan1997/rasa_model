...
def conversation1():
    ...
    for text in texts:
        ...
        try:
            ...
            # tag_count = 0
            #
            # fromto_matches = re.findall(r'(?:year|month|week|day|hour|minute|second|range).*?到.*?(?:year|month|week|day|hour|minute|second|range)',text)
            #
            # single_matches = re.findall(r'year|month|week|day|hour|minute|second|range', text)
            #
            # without_city_text = text.replace('city', '')
            # city_count = (len(text) - len(without_city_text)) / 4
            # print(fromto_matches, single_matches)  # test
            #
            # if len(fromto_matches) + len(single_matches) == 1 and city_count == 1:
            #     print('hello')
            #     if fromto_matches:
            #         print('fromto')
            #     elif single_matches:
