...
def zh_dates_cities(text):
    ...
    if matched_indexes and city_indexes:
        ...
        if matched_indexes[-1] < city_indexes[0] or city_indexes[-1] < matched_indexes[0]:
            ...
            # until
            found_dates, text, matched_time_lines = zh_get_until(found_dates, text, matched_time_lines)
            after_until_text = text  # test
            # single
            found_dates, text, matched_time_lines = zh_get_single(found_dates, text, matched_time_lines)
...
def zh_get_until(found_dates, text, matched_time_lines):
    matches = re.findall(
        r'(?:year|month|week|day|hour|minute|second|range).*?到.*?(?:year|month|week|day|hour|minute|second|range)',
        text)
    # tag1到tag2
...
def zh_get_single(found_dates, text, matched_time_lines):
    matches = re.findall(r'year|month|week|day|hour|minute|second|range', text)
    # 單獨
    for match in matches:
        print(f'>> 開始處理單獨標籤: {match}')
...
def en_get_single(found_dates, text, matched_time_lines):
    matches = re.findall(r'year|month|week|day|hour|minute|second|range', text)
    ...
    for match in matches:
        ...
        check_text = text[:text.index(match)]
        print(f'check_text = {check_text}')

        text = text[text.index(match) + len(match):]

        print(f'en get single text = {text}')
...
def en_dates_cities(text):
    ...
    if matched_indexes and city_indexes:
        ...
        if matched_indexes[-1] < city_indexes[0] or city_indexes[-1] < matched_indexes[0]:
            ...
            # single
            found_dates, text, matched_time_lines = en_get_single(found_dates, text, matched_time_lines)
            after_single_text = text  # test
...
for text in texts:
    en_dates_cities(text)
    print('--------------------------------------------------------------------------')
    # dates_cities_indexes = en_dates_cities(text)
    # print('--------------------------------------------------------------------------')
    # print(text)
    # print(dates_cities_indexes)
    # for dates_cities_index in dates_cities_indexes:
    #     print(data[dates_cities_index]['cit'], data[dates_cities_index]['pdt'])
    # print('--------------------------------------------------------------------------')

# for text in texts:
#     dates_cities_indexes = zh_dates_cities(text)
#     print('--------------------------------------------------------------------------')
#     print(text)
#     print(dates_cities_indexes)
#     for dates_cities_index in dates_cities_indexes:
